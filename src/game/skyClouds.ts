...
export interface CloudFrameInfo {
  ...
  storm: number;
  /** 0..1 — player's winter-biome factor: the deck turns icy white-blue. */
  winter?: number;
}
...
const CUMULUS_HEIGHT = 2000; // raised: a tall, towering deck far overhead
const CIRRUS_HEIGHT = 3800; // high wispy veil
...
const WINTER_LIT = new THREE.Color(0xf2f9ff); // sunlit snow-cloud tops
const WINTER_SHADOW = new THREE.Color(0x9db3c6); // cold blue-grey bellies
...
export function createSkyClouds(
  ...
  return {
    update(info: CloudFrameInfo) {
      ...
      // winter biomes bleach the deck toward icy whites (after storm so a
      // squall over a frozen zone still reads cold, not wet-grey)
      const winter = THREE.MathUtils.clamp(info.winter ?? 0, 0, 1);
      if (winter > 0.001) {
        lit.lerp(WINTER_LIT, winter * 0.75);
        shadow.lerp(WINTER_SHADOW, winter * 0.75);
      }
      const silver =
